...
// Return dtw distance between two 2-dimensional sequences
// length of sequences, second dimension length (must be identical), sequences
float dtw_cost(int a_len, int b_len, int d, float *a, float *b) {
    ...
    return sqrtf(cost);                    // return square root
}
...
int dtw_path(int a_len, int b_len, int d, float *a, float *b, unsigned short *wp) {
    // D holds accumulated DTW cost
